...
- Runtime loss retains the old terminal screen; input is disabled and its UIKit first responder is resigned. The per-attempt input pipe discards queued bytes on stop, rejects subsequent offline callbacks and cannot be rebound to a replacement transport. Deterministic suspended-writer tests verify that only deliberately supplied new-attempt input reaches the replacement. Bytes already handed to an old transport have ambiguous delivery and are never replayed. A new candidate replaces it only after accepted remote startup and output. Old local scrollback remains visible during retry, but is not merged into the new terminal buffer; tmux copy mode remains the remote-history source.
