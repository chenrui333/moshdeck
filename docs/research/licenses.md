...
The arm64 simulator candidate subsequently built successfully: 19,185,816 bytes, SHA-256 `8097341621b8b0552fb9b9a30f7801b3de0c7fb075d4fe4158aac07a2a14189e`, with zero matches for the same gettext symbol searches. Both arm64 variants were packaged into an isolated XCFramework and consumed by a copy of MoshDeck at `822d04c` using a local package reference. The repository's dependency pin was not changed.

The first consumer invocation built successfully but selected a nonexistent test class and executed **zero tests**; it is not a regression pass. The corrected invocation explicitly selected `FrontendTests/testRealTerminalParserAndInputPaths` and `FrontendTests/testTerminalKeyboardDismissalAndExpansion`: both passed, respectively 6.762 and 12.871 seconds, with two tests, zero failures and `TEST SUCCEEDED`. These are simulator results. They do not establish physical SSH recovery, device rendering, locale compatibility beyond the fixtures, or distribution readiness. A device consumer build, reproducible artifact delivery, remaining attribution review and physical regression are still required before adopting the candidate.

The isolated arm64 iOS Release consumer also passed `xcodebuild build`. Its final executable is 17,700,416 bytes, SHA-256 `2b9b353f7dd36acd0905718b3deb594cfb6be90cb09a1fa9ce9ad7b92b8896ef`. Inspection of all 13,036 lines from `nm -g` found zero matches for the five gettext symbol terms above. This closes the device consumer link check for the candidate. The app's pinned release artifact still contains libintl until this candidate is deliberately integrated through a reproducible dependency delivery path; the old IPA must not be described as fixed. No physical-device test or upload was performed for this candidate.

The integrated source-built distribution IPA was also inspected: strict signature verification passed and its final executable has no gettext symbol matches. This removes the previously demonstrated libintl linkage from this new candidate IPA, not from historical exports. Wrapper shell resources were inspected separately: the bash/zsh integration files declare the wrapper author's MIT rewrite, and bash-preexec has its accompanying MIT license resource. References to GPL in those headers describe the upstream implementations the wrapper says it replaced; a keyword hit alone must not be treated as a GPL license declaration for the replacement. Remaining symbol-font attribution is not closed by this resource check.

## Additional symbol-font notices — September 9

`AdditionalSymbolFonts.txt` includes four component notices with source revisions and license hashes in [additional-font-notices.json](additional-font-notices.json). The vendored `font-awesome-extension.ttf` and `Unicode_IEC_symbol_font.otf` were downloaded from the exact Nerd Fonts revision and compared with the corresponding files at the recorded component revisions: both are byte-identical. Their MIT notices therefore have direct artifact provenance, rather than only a matching project name.

Font Logos metadata identifies version 1.3.0; its notice is taken from that upstream release (Unlicense). Nerd Fonts' Devicons README identifies `2.16.0.custom`; the included MIT notice is taken from upstream v2.16.0. These latter notices use version evidence, not byte-identical font evidence. The font-name records alone contained no license text for the four inspected source fonts. Nerd Fonts' own Symbols Only source font has an MIT notice, but it does not replace component notices. Seti/original SVG provenance remains a separate open attribution item.
