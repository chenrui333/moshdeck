...
## Current shipped beta — September 12, 2026

TestFlight 0.0.1 (1) uses the local package generated by `Spike/scripts/prepare-ghostty.py`, not the original downloaded XCFramework. The exact wrapper/core revisions, patch and reproduction process are recorded below and in [TestFlight evidence](../testflight.md). The source-built Release executable passed the gettext symbol check: the earlier libintl finding applies to the superseded binary, not this uploaded artifact.

The uploaded notice-inclusive IPA contains the additional symbol-font and Seti notices described at the end of this record; those resources were checked byte-for-byte after export. This closes the specific missing-notice and original-libintl preparation gates recorded here. It is not a blanket clearance of every logo's provenance or future dependency versions. Apple processing and signing are not license audits. Physical regression of the newly source-built terminal remains beta acceptance work.

The table is the original ecosystem comparison, including unselected alternatives. The dated narrative below preserves the investigation as it happened; phrases such as “current iOS build” in earlier checkpoints refer to that checkpoint's artifact.
