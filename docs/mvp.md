...
The Mac can host multiple tmux sessions, windows and panes. MoshDeck currently has one saved profile and one active connection. tmux's native session picker (Ctrl-B, then S) can switch the current client, but that switch does not update MoshDeck's saved target: after interruption, reconnect attaches to the session named in the profile. For predictable recovery, explicitly disconnect and change the saved Session field before connecting to another workspace. Switching between at least two tmux sessions from the physical phone is an MVP acceptance gate; the existing Ctrl-B control and normal tmux picker are sufficient. The owner subsequently requested native browsing and direct swiping. Build 4 offers a Sessions button and horizontal terminal paging through the same alphabetical list, without wrapping. Both call one client-specific tmux switch operation on the existing SSH connection. Only confirmed success saves a new reconnect target. Manual tmux switches are observed on refresh, but do not automatically rewrite the saved target. The chip says Sessions to avoid implying continuously observed current-session state. Physical swipe acceptance is pending; the owner waived direct hardware checks for this beta iteration. Multiple simultaneous app terminals remain post-MVP.
...
Release uses a compact host/status header with tmux actions, Compose and overflow. The saved **Reconnect target** is shown in the tmux menu/details, not presented as an observed live session. Sessions opens the transient native session picker. Horizontal dragging previews and selects an adjacent session; short or cancelled drags perform no remote action. Ctrl-B and Mac attachment help are in overflow. Its terminal-picker fallback sends the default Ctrl-B, then lowercase s sequence; custom tmux prefixes remain manual. The header exposes Show Keyboard when terminal focus is dismissed. A single keyboard accessory provides Esc, sticky Ctrl, Tab, arrows and Hide Keyboard. Ctrl+C is available by arming Ctrl and typing c.
